...
namespace zjh.SSLY.BLL.Info
{
    ...
    {
        private IBillGoodsReceiptHdrRepository dal = new BillGoodsReceiptHdrRepository();
        public bool Save(BillGoodsReceiptHdr hdr, List<BillGoodsReceiptDtl> dtls)
        {
            IBillGoodsReceiptDtlRepository dalDtl = new BillGoodsReceiptDtlRepository();
            string formno = dal.Save(hdr);
            foreach (BillGoodsReceiptDtl dtl in dtls)
            {
                dtl.Uid = "1";
                dtl.Formno = formno;
                dtl.Active = true;
